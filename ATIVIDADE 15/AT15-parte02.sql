...
-- 1. Crie uma view chamada "vw_cliente_detalhes" que selecione o nome, email, telefone e
-- a cidade dos clientes.
...
-- 2. Crie uma view chamada "vw_produtos_estoque" que selecione o nome, categoria e quanti-
-- dade em estoque dos produtos com estoque maior que 100.
...
-- 3. Crie uma view chamada "vw_clientes_nascimento" que selecione o nome, ano de nascimento
-- e mês de nascimento dos clientes.
CREATE VIEW vw_cliente_nascimento AS
SELECT nome_cliente, YEAR(data_nascimento) AS ano_nascimento, MONTH(data_nascimento) AS mes_nascimento
FROM tb_cliente;

SELECT * FROM vw_cliente_nascimento;

-- 4. Crie uma view chamada "vw_produtos_precos" que selecione o nome do produto e um novo 
-- campo "lucro" com a diferença entre o preço de venda e o preço de compra.
CREATE VIEW vw_produtos_precos AS
SELECT nome_produto, (preco_venda -preco_compra) AS lucro
FROM tb_produto;

SELECT * FROM vw_produtos_precos;

-- 5. Crie uma view chamada "vw_categorias" que exiba as categorias e a contagem de produtos
-- em cada categoria.
...
-- 6. Usando a view "vw_cliente_detalhes", selecione todos os clientes que moram na cidade de 
-- 'São Paulo'.
SELECT *
...
-- 7. Usando a view "vw_produtos_estoque", selecione o nome e a categoria dos produtos que pos-
-- suem mais de 150 unidades em estoque.
...
-- 8. Usando a view "vw_clientes_nascimento", selecione o nome dos clientes nascidos no mês de
-- fevereiro.
SELECT nome_cliente
FROM vw_cliente_nascimento
...
SELECT * FROM vw_categoria 
WHERE qtd_produtos > 10;

-- PARTE 2 ----------------------------------------------------------------------------------------

-- 11. Crie uma VIEW chamada "vw_clientes_aniversariantes" que exibe o nome do cliente, telefone, e 
-- mês de nascimento, para clientes que fazem aniversário no primeiro semestre (janeiro a junho).

CREATE VIEW vw_clientes_aniversariantes AS
 SELECT nome_cliente, telefone_cliente, MONTH(data_nascimento) AS mes_nascimento
 FROM tb_cliente 
...
-- 13. Crie uma VIEW chamada "vw_clientes_mais_lucrativos" que exibe o nome do cliente, telefone, cidade,
-- estado e a soma do lucro bruto de todos os produtos com estoque menor que 50 vendidos para clientes que
-- moram em estados do Sudeste (SP, RJ, MG, ES).

 CREATE VIEW vw_clientes_mais_lucrativos AS 
 SELECT c.nome_cliente, c.telefone_cliente, c.cidade, c.estado, SUM(p.preco_venda - p.preco_compra) AS lucro_total 
 FROM tb_cliente c 
 JOIN tb_produto p ON p.estoque < 50 
 WHERE c.estado IN ('SP', 'RJ', 'MG', 'ES') 
 GROUP BY c.id_cliente; 
 
 SELECT * FROM vw_clientes_mais_lucrativos; 

-- 14. Usando a view "vw_clientes_aniversariantes", selecione os nomes dos clientes que fazem aniversário 
-- em um mês ímpar e ordene pelo mês de nascimento. Utilize uma condição para exibir o mês por extenso 
-- (ex: 'Janeiro', 'Março', etc.).

 SELECT nome_cliente, telefone_cliente,
		CASE 
            WHEN mes_nascimento = 1 THEN 'Janeiro' 
            WHEN mes_nascimento = 3 THEN 'Março' 
            WHEN mes_nascimento = 5 THEN 'Maio' 
		END AS mes_aniversario 
FROM vw_clientes_aniversariantes 
WHERE mes_nascimento IN (1, 3, 5) 
ORDER BY mes_nascimento; 

-- 15. Usando as views "vw_produtos_lucro" e "vw_clientes_mais_lucrativos", selecione os nomes dos clientes
-- e o lucro total de produtos da categoria 'Alimentos' vendidos para esses clientes, mas apenas para aqueles 
-- que geraram lucro acima de 100. Ordene pelo lucro em ordem decrescente.

SELECT c.nome_cliente, SUM(p.lucro_bruto) AS lucro_total 
FROM vw_clientes_mais_lucrativos c 
JOIN vw_produtos_lucro p ON p.categoria = 'Alimentos'
GROUP BY c.nome_cliente HAVING lucro_total > 100 
